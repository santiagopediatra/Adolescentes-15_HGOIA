...
| 4 | `R/04_punto6_atencion_prenatal.R` | CSV y RDS principal | `output/prenatal/`: regla de contactos, categorías, modelos binario/ordinal, thresholds, diagnósticos y odds proporcionales |
...
| 6 | `R/06_punto8_diagnosticos_priors.R` | RDS principal y `output/consultas_prenatales/ppc_gaussiano.csv` | `output/diagnosticos/`: diagnósticos completos, parámetros posteriores, PPC y sensibilidad a priors |
| 7 | `R/07_punto9_heterogeneidad_temporal.R` | RDS principal | `output/heterogeneidad_temporal/`: interacción, estimaciones por período y comparación LOO |
| 8 | `R/08_punto4_sensibilidad_rezago_escolar.R` | CSV | `output/rezago_escolar/`: regla principal y alternativa, diagnósticos, PPC y Tabla S14 |
| 9 | `Figura2_flujograma_STROBE.R` | Ninguna; usa conteos codificados que deben cotejarse | `manuscript/media/media/image2.png` |
| 10 | Scripts originales faltantes de Figuras 1 y 3 | Por recuperar | `image1.png` e `image3.jpeg` |
| 11 | `quarto render manuscript/Adolescentes_19_julio.qmd` | QMD, CSL, bibliografía y figuras | DOCX; las tablas del QMD son estáticas y deben cotejarse manualmente |
...
- Los pasos 4–7 requieren `resultados_RBSMI_DEFINITIVO_CORREGIDO.rds`, producido en el paso 1.
